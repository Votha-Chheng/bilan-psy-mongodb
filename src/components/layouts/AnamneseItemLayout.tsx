...
const AnamneseItemLayout: FC<AnamnesItemsLayoutProps> = ({ children, partieAnamnese, setPartieAnamnese }) => {
  ...
  return (
    <article className="min-w-full px-5">
      ...
      <div className={`${partieAnamnese === 0 ? "justify-end": partieAnamnese === 6 ? "justify-start" : "justify-between"} flex gap-x-2 w-full mb-5`}>
        {
          partieAnamnese !== 0 &&
          <Button type='button' onClick={()=> setPartieAnamnese(partieAnamnese-1)}>
            <SkipBack size={19} className='mr-2'/> {nomPartieLeft} 
          </Button>
        }
        {
          partieAnamnese !== partiesListes.length-1 &&
          <Button type='button' onClick={()=> setPartieAnamnese(partieAnamnese+1)}>
